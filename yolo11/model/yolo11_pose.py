...
try:
    from .modules import ConvModule, C2PSA, YoloStage, SPPF, Upsample, Concat, Pose
except:
    from  modules import ConvModule, C2PSA, YoloStage, SPPF, Upsample, Concat, Pose


# Yolo11 model for the general object detector
class Yolo11Pose(nn.Module):
    def __init__(self, model_scale: str = "l", num_classes: int = 1):
        ...
        # --------- Task Head ---------
        self.model.append(
            Pose(nc=self.num_classes, kpt_shape=[self.num_kepoints, self.num_kptdims], ch=[int(256 * width), int(512 * width), int(512 * width * ratio)]),  # layer-23
        )
...
if __name__ == "__main__":
    ...
    model = Yolo11Pose(model_scale="s")
