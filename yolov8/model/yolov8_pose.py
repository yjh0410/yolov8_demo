...
try:
    from .modules import ConvModule, YoloStage, SPPF, Upsample, Concat, Pose
except:
    from  modules import ConvModule, YoloStage, SPPF, Upsample, Concat, Pose


# Yolov8 model for the general object detector
class Yolov8Pose(nn.Module):
    def __init__(self, model_scale: str = "l", num_classes: int = 1):
        super().__init__()
        self.model_scale = model_scale
        self.num_classes = num_classes
        self.num_kepoints = 17
        self.num_kptdims = 3
        ...
        # --------- Task Head ---------
        self.model.append(
            Pose(nc=self.num_classes, kpt_shape=[self.num_kepoints, self.num_kptdims], ch=[int(256 * width), int(512 * width), int(512 * width * ratio)]),  # layer-22
        )
    ...
    def _forward_impl(self, x):
        ...
        # ------------------ Task head ------------------
        output = self.model[22](pyramid_feats)
...
if __name__ == "__main__":
    ...
    model = Yolov8Pose(model_scale="s")
