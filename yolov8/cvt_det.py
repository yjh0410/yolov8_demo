...
model_scale = "s"
ckpt = torch.load(f'checkpoints/official/yolov8{model_scale}.pt', map_location="cpu")
yolov8 = ckpt.pop("model")
yolov8 = yolov8.float()

my_yolov8 = Yolov8Detector(model_scale=model_scale, num_classes=80).eval()
my_yolov8.load_state_dict(yolov8.state_dict()) 
torch.save({"model": my_yolov8.state_dict()}, os.path.join("checkpoints/yolov8{}_det_ckpt.pth".format(model_scale)))
my_yolov8.load_state_dict(torch.load(os.path.join("checkpoints/yolov8{}_det_ckpt.pth".format(model_scale)), map_location="cpu")["model"])
