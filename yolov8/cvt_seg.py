...
model_scale = "l"
ckpt = torch.load(f"checkpoints/official/yolov8{model_scale}-seg.pt", map_location="cpu")
...
my_yolov8 = Yolov8Segmentor(model_scale=model_scale, num_classes=80, num_masks=32).eval()
my_yolov8.load_state_dict(yolov8.state_dict()) 
torch.save({"model": my_yolov8.state_dict()}, os.path.join("checkpoints/yolov8{}_seg_ckpt.pth".format(model_scale)))
my_yolov8.load_state_dict(torch.load(os.path.join("checkpoints/yolov8{}_seg_ckpt.pth".format(model_scale)), map_location="cpu")["model"])
